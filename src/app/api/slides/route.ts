...
// POST: Add a new slide (upload slide media)
export async function POST(req: NextRequest) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'src/public/slides');
    ...
    if (slideFiles.length > 0) {
      const slideUploadPromises = slideFiles.map(async (file: File) => {
        ...
        slidePaths.push({ name: uniqueFileName, path: `/api/slides/file/${uniqueFileName}` });
      });
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ media: slides }, { status: 200 }); // Updated response key
  } catch (error) {
